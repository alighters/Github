...
public class UserRepository {
    ...
    ApiServiceProxy<LoginService> mServiceProxy;

    @Inject
    public UserRepository(ApiServiceProxy<LoginService> apiServiceProxy, LoginCache loginCache) {
        mLoginCache = loginCache;
        mServiceProxy = apiServiceProxy;
    }

    /**
     * 执行用户的登陆
     */
    public Observable<AuthEntity> login(String name, String password) {
        return mServiceProxy.getProxy(LoginService.class)
            .login(NetConstant.MD_APP_KEY, NetConstant.MD_APP_SECRET, LoginService.GrantType.PASSWORD, name,
                password)
